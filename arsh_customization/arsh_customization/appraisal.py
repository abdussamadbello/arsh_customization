...
class ArioshAppraisal(Appraisal):
	...
	def calculate_final_score(self):
		...
		final_score = (flt(self.total_score) * kra_weight / 100 + 
				 flt(self.avg_feedback_score) * feebback_weight / 100 + 
				 flt(self.self_score) * self_weight / 100) / (kra_weight + feebback_weight + self_weight)
